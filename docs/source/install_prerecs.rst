...
5. The files with the coefficients necessary to calculate the collision-induced absorption (CIA) for O:math:`_2` collisions with other atmospheric particles. If needed, other coefficients are available on the `HITRAN database <https://www.hitran.org/cia/>`_. 
...
To install each package, activate the environment doing ``>> conda activate <my_env>`` and running:
