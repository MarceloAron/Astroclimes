...
if $env_exists; then
    ...
    conda env update --name "$envname" --file environment-linux64-py310.yml
else
    echo "Will create conda environment '$envname'"
    conda env create --name "$envname" --file environment-linux64-py310.yml
fi
...
# If we installed a conda environment, then this will just install ginput in develop mode so that it can be
