...
'''
	Setup script for finding the best-fit molecular abundance values with Astroclimes when running with SLURM
'''

## Reading command line argument specifying index of file to be analysed
...
filename_mcmc_results = MCMC_directory + f'results_mcmc_{file_index}.txt'
...
## Limiting the files to go through to run in batches
list_science_spectra = list_science_spectra[file_index:(file_index+1)]
...
orbpars.T0 = 2455652.108	# Planet mid-transit time, in BJD, from Brogi et al. (2012)
...
## Choose the "deep line threshold", which defines the points below which transmission will be masked out of the analysis when looking for the best-fit telluric models
