...
main_telrem_directory = home_directory+'telluric_removal/TauBoo/2018_03_26_Kp_50/molecfit/'

Kp = 50
Vsys = -16.4 

#bad_walkers_AC = [[],[6,7],[],[],[],[],[],[],[],[],[2],[],[],[],[2],[]]
...
for n, scale_factor in enumerate(scale_factors):
	...
	dirname_MCMC_AC = dirname+'MCMC_telrem_MOLECFIT/'
	...
	print(scale_factor, bad_walkers)
	#bad_walkers = bad_walkers_AC[n]
	...
	#txt = open(main_telrem_directory+'results_inj_rec_mcmc_ac.txt', 'a')
	txt = open(main_telrem_directory+'results_inj_rec_mcmc_molecfit.txt', 'a')
	txt.write(text)
	txt.close()
	...
	logp = sampler.get_log_prob()
	logpflat = sampler.get_log_prob(discard=discard, thin=thin, flat=True)
	#bad_walkers = np.where(logp[-1,:] < (np.median(logp[-1,:]) - 2*np.std(logp[-1,:])))[0]
	bad_walkers = np.where(logp[-1,:] < (np.median(logp[-1,:]) - 3*np.std(logp[-1,:])))[0]
	samples, flat_samples, logp, logpflat = funcs.delete_bad_walkers(samples, flat_samples, logp, logpflat, bad_walkers, nsteps, nwalkers, discard, thin)
	...
	text =	(f"{scale_factor:<10.2f} \t {nc_PCA:<5.0f} \t" +
			...
			f"{logp_med:10.4f} \t {u_logp_med:10.4f}\n")
	txt = open(main_telrem_directory+f'results_inj_rec_mcmc_pca_nc_{nc_PCA}.txt', 'a')
	txt.write(text)
	txt.close()
