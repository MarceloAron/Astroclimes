...
def get_ggg2020_atm_profiles(date_obs, dirname, molecs, hgt_bot=0., use_vmr_file=False):
	'''
	...
	if use_vmr_file:
		...
		Xs_atm = dict.fromkeys(molecs, {})
		Xs_atm_WET = dict.fromkeys(molecs, {})
		map_header = np.loadtxt(map_file, skiprows=10, max_rows=1, dtype=str, unpack=True, delimiter=',').tolist()
		for molec in molecs:
			if molec == 'N2':
				continue
			elif molec == 'H2O' or molec == 'O2':
				Xs_atm_WET[molec] = 1e6*np.loadtxt(map_file, skiprows=12, usecols=(map_header.index(molec.lower())), unpack=True, delimiter=',')
			elif molec == 'CH4':
				Xs_atm_WET[molec] = 1e-3*np.loadtxt(map_file, skiprows=12, usecols=(map_header.index(molec.lower())), unpack=True, delimiter=',')
			else:
				Xs_atm_WET[molec] = np.loadtxt(map_file, skiprows=12, usecols=(map_header.index(molec.lower())), unpack=True, delimiter=',')

		## Converting from WET mole fractions to DRY mole fractions
		## [H2O]_dry = (1/[H2O]_wet - 1)^-1 and then [gas]_dry = [gas]_wet * (1 + [H2O]_dry)
		Xs_atm['H2O'] = 1e6*(1e6/Xs_atm_WET['H2O'] - 1)**(-1)
		for molec in molecs:
			if molec == 'N2':
				Xs_atm[molec] = np.array([780800.0]*len(hgt_atm))
			else:
				Xs_atm[molec] = Xs_atm_WET[molec]*(1+1e-6*Xs_atm['H2O'])
